...
! Adapted by Debao Guan, University of Glasgow, 2020
! to compile (windows): gfortran b1b2_22.f90 -o b1b2_rho.exe
! run b1b2_rho.exe
...
! download from https://people.sc.fsu.edu/~jburkardt/f77_src/sphere_triangle_quad/sphere_triangle_quad_prb.f


  implicit none
  
  real, dimension(15000,3) :: array1_node
  integer, dimension(25000,3) :: array2_element
  integer :: N_node,N_element,i

  real ( kind = 8 ) rho_n, rho_inter, rnormal
  real ( kind = 8 ) v1(3),vv(3)
  real ( kind = 8 ) v2(3),N_n(3)
  real ( kind = 8 ) v3(3)

  
  open(12, file="sphere_node1280.txt")
! read in values
  read(12,*) N_node
  write(*,*) N_node
  do i=1,N_node
    read (12,*) array1_node(i,1), array1_node(i,2), array1_node(i,3)
!    write(*,*) array1_node(i,:)
  end do
  close(12)
  
  
  open(13, file="sphere_element1280.txt")
! read in values
  read(13,*) N_element
  write(*,*)N_element
  do i=1,N_element
    read (13,*) array2_element(i,1), array2_element(i,2), array2_element(i,3)
!    write(*,*) array2_element(i,:)
  end do
  close(13)
...
!  call timestamp ( )
  ...
  open(1, file = 'PDF1280_b1b2_22_human.txt')  
  write(1,*) N_element/2
...
! what is it?
  ...
  do i=1,N_element
     ...
     v1(2)=array1_node(array2_element(i,1),2)
     v1(3)=array1_node(array2_element(i,1),3)
     v2(1)=array1_node(array2_element(i,2),1)
     v2(2)=array1_node(array2_element(i,2),2)
     v2(3)=array1_node(array2_element(i,2),3)
     v3(1)=array1_node(array2_element(i,3),1)
     v3(2)=array1_node(array2_element(i,3),2)
     v3(3)=array1_node(array2_element(i,3),3)

     vv(1)=(v1(1)+v2(1)+v3(1))/3.0d0
     vv(2)=(v1(2)+v2(2)+v3(2))/3.0d0
     vv(3)=(v1(3)+v2(3)+v3(3))/3.0d0
     ...
     call test03 (v1,v2,v3,rho_n)
     rho_inter=rho_inter+rho_n
     if (rho_n .gt. 0.0) then
     	write(1,*) rho_n/rnormal, N_n(1), N_n(2), N_n(3)
     end if
  ...
  write(*,*)rho_inter/rnormal,(rho_inter/rnormal-1.0)*100
  write(*,*)rho_inter
...
subroutine test03 (v1,v2,v3,rho_n)
...
!    polynomial term.
!
! purpose: compute equation 3, without including 2pi*2pi in the denominator 
! only count the vectors with positive coordinate in z direction
  ...
  real ( kind = 16 ) value, rho,b1,b2,b3,b4,theta, phi
  real ( kind = 8 ) x(3)
...
! set Md=(0 0 1)
  ...
  b1=2.00d0  !in plane dispersion 
  b2=2.00d0	 !out of plane dispersion

  b3=2.279585302336d0   ! bessel function (2)
  b4=2.279585302336d0   ! bessel function (2)
! this can be calculated using Matlab function besseli(0,2)

! assume the mean fibre direction is at the first axis
! see equation (2) in the main text, theta = cos(theta)
  theta=(1.0d0*x(1)+0.0d0*x(2)+0.0d0*x(3))/sqrt(x(1)**2+x(2)**2+x(3)**2)
! see equation (2) in the main text for detail, phi = cos(phi)
  phi=x(3)/sqrt(x(2)**2+x(3)**2)
  if (x(3) .lt. 0.0) then
	rho=0.0d0
  else
!	rho=exp(b1*cos(2*theta))/1.7667d0*exp(b2*cos(2*phi))/1.4041d0
!   cos(2*theta) = 2 cos(theta)**2 -1 
!   theta = cos(theta) 
!   denomerator is 2*pi*I0(b), I0 is the modeified Bessel function 
!   of the first kind of order zero
  	rho=exp(2.0d0*b1*theta**2-b1)/b3*exp(2.0d0*b2*phi**2-b2)/b4
  end if
!  rho=1.0d0


  polyterm_value_3d = rho
!  write ( *, * ) rho, ERF(sqrt(2*b))
